...
- Result: `88 passed, 2 failed, 11 skipped`
- Current failures:
...
| DCE primary models | `dce/model_tofts*.m`, `dce/model_patlak*.m`, `dce/model_extended_tofts*.m` | Implemented in `python/dce_models.py` and wired into pipeline | primary | Must be strict-gated for merge readiness. |
...
| Parametric core model math | `parametric_scripts/fitParameter.m` (`t2_linear_fast`, `t1_fa_linear_fit`) | Implemented in `python/parametric_models.py` | done | Covered by Python unit and contract-parity tests. |
| Parametric T1 mapping workflow | `parametric_scripts/custom_scripts/T1mapping_fit.m`, `parametric_scripts/calculateMap.m` | Not ported end-to-end | primary | Needed for real workflow replacement and user testing. |
