...
| Parametric T1 mapping workflow | `parametric_scripts/custom_scripts/T1mapping_fit.m`, `parametric_scripts/calculateMap.m` | Partial (`python/parametric_pipeline.py`, `python/parametric_cli.py`, `run_parametric_python_cli.py`) | primary | VFA linear/nonlinear/two-point fit types are available with fixture + BIDS-based naming/integrity tests; optional B1-scaled FA support (`b1_map_file` or auto-detected `B1_scaled_FAreg.nii(.gz)`) is now implemented; remaining MATLAB-path behavior is still pending. |
