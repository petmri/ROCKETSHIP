...
class ParametricT1Config:
    ...
    def validate(self) -> None:
        if self.fit_type.strip().lower() != "t1_fa_linear_fit":
            raise ValueError("Only fit_type='t1_fa_linear_fit' is currently supported")
...
def _resolve_flip_angles_and_tr_ms(config: ParametricT1Config, n_flips: int) -> Tuple[np.ndarray, float, List[Path]]:
    sidecars: List[Path] = []
...
def run_parametric_t1_pipeline(
    ...
    flip_angles_deg, tr_ms, sidecars = _resolve_flip_angles_and_tr_ms(config, n_flips)
    _emit_event(
        event_callback,
        "inputs_resolved",
        n_flips=n_flips,
