...
- Part E statistical core port started: Python helpers now cover MATLAB-style model support checks, SSE extraction, f-test, AIC/relative-likelihood comparison, ROI CSV outputs, and voxel-vector-to-volume reconstruction with unit coverage (`python/dce_postfit_analysis.py`, `tests/python/test_dce_postfit_analysis.py`).
