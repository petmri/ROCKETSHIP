...
- CI workflow now separates Python-only checks from MATLAB-backed parity checks: parity runs in dedicated `parity_checks` after generating downsample MATLAB baseline maps in-job, `python_portability` includes `ubuntu-22.04`, and unified `matlab_checks` now runs the `R2020b`/`R2022a`/`latest` matrix on both pushes and pull requests (with in-progress run cancellation per ref) (`.github/workflows/run_DCE.yml`).
