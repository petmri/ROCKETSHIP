#!/usr/bin/env python3
"""Set up the ROCKETSHIP Python environment: virtual environment, dependencies,
Gpufit/Cpufit acceleration, and the per-GUI launcher scripts."""
...
def _warn(message: str) -> None:
    print(f"[install] WARNING: {message}", flush=True)
...
# Generated by install.py. Launches the ROCKETSHIP __LABEL__ GUI inside the project
# virtual environment. Re-run "python3 install.py" to regenerate.
set -euo pipefail
...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
    echo "Usage: $(basename "${BASH_SOURCE[0]}") [gui args...]"
    echo "Launches the ROCKETSHIP __LABEL__ GUI in the project virtual environment."
    exit 0
fi

if [ ! -f "$ACTIVATE" ]; then
    echo "rocketship: virtual environment not found at $VENV_DIR" >&2
    echo "rocketship: run 'python3 install.py' from $REPO_ROOT first." >&2
    exit 1
fi
...
exec python "$REPO_ROOT/__GUI__" "$@"
"""


LAUNCHER_BAT = """@echo off
REM Generated by install.py. Launches the ROCKETSHIP __LABEL__ GUI inside the project
REM virtual environment. Re-run "python install.py" to regenerate.
setlocal
set "REPO_ROOT=%~dp0"
set "VENV_DIR=__VENV_DIR__"
...
if not exist "%ACTIVATE%" (
    echo rocketship: virtual environment not found at %VENV_DIR% 1>&2
    echo rocketship: run "python install.py" from %REPO_ROOT% first. 1>&2
    exit /b 1
)
call "%ACTIVATE%"
python "%REPO_ROOT%__GUI__" %*
exit /b %ERRORLEVEL%

:usage
echo Usage: %~nx0 [gui args...]
echo Launches the ROCKETSHIP __LABEL__ GUI in the project virtual environment.
exit /b 0
"""


# One launcher per GUI, as (stem, GUI entry point, human label). A single launcher taking a
# subcommand meant the parametric GUI existed only as an argument nobody was told about.
LAUNCHERS = (
    ("rocketship_dce", "run_dce_python_gui.py", "DCE"),
    ("rocketship_parametric", "run_parametric_python_gui.py", "Parametric T1"),
)

# Written by earlier releases, which had one launcher taking [dce|parametric]. Removed on
# re-install so a stale script cannot keep launching an interface that has moved on.
LEGACY_LAUNCHERS = ("rocketship.sh", "rocketship.bat")


def _write_gui_launchers(repo_root: Path, venv_python: Path) -> List[Path]:
    """Write one shell wrapper per GUI, each activating the venv and launching its own."""
    venv_root = _venv_root_from_python(venv_python)
    if os.name == "nt":
        try:
            venv_expr = "%REPO_ROOT%" + str(venv_root.relative_to(repo_root))
        except ValueError:
            venv_expr = str(venv_root)
        template, suffix = LAUNCHER_BAT, ".bat"
    else:
        try:
            venv_expr = '"$REPO_ROOT"/' + venv_root.relative_to(repo_root).as_posix()
        except ValueError:
            venv_expr = shlex.quote(str(venv_root))
        template, suffix = LAUNCHER_SH, ".sh"

    for name in LEGACY_LAUNCHERS:
        stale = repo_root / name
        if stale.exists():
            stale.unlink()
            _log(f"Removed superseded launcher: {stale}")

    written: List[Path] = []
    for stem, gui, label in LAUNCHERS:
        launcher = repo_root / f"{stem}{suffix}"
        body = (
            template.replace("__VENV_DIR__", venv_expr)
            .replace("__GUI__", gui)
        ...
        _log(f"Wrote {label} GUI launcher: {launcher}")
...
def _print_post_install_next_steps(
    repo_root: Path,
    venv_python: Path,
    launchers: Optional[Sequence[Path]] = None,
) -> None:
    venv_root = _venv_root_from_python(venv_python)
    if os.name == "nt":
    ...
    step = 1
    if launchers:
        # Name both. The parametric GUI previously existed only as a subcommand of one
        # launcher, which meant it was never mentioned here and nobody found it.
        print(f"  {step}) Launch a GUI (from {repo_root}):")
        commands = [
            path.name if os.name == "nt" else f"./{path.name}" for path in launchers
        ]
        width = max(len(command) for command in commands)
        for command, (_stem, _gui, label) in zip(commands, LAUNCHERS):
            print(f"       {command.ljust(width)}   {label} GUI")
        step += 1
    print(f"  {step}) Activate env: {activate_cmd}")
    step += 1
    print(f"  {step}) Run Benchmarks: python tests/python/run_dce_benchmark.py --dataset-root <bids_root>")
    step += 1
    print(f"  {step}) Run Python CLI: python run_dce_python_cli.py --help")
    step += 1
    print(f"  {step}) Run MATLAB CLI: matlab -batch \"cd(pwd); run_dce_cli('<subject_source_path>','<subject_tp_path>');\"")
...
def _check_matlab_install(
    repo_root: Path,
    matlab_cmd: str,
    host_key: str,
    skip: bool,
) -> Tuple[Dict[str, str], Dict[str, str]]:
    """Verify the MATLAB side of the install; return (symbol, runtime) health rows.

    A missing MATLAB installation is a warning, not a failure: the Python pipelines do
    not need it. Only a MATLAB that is present but cannot see the required Gpufit
    symbols raises.
    """
    if skip:
        detail = "MATLAB MEX install skipped by user"
        _log(detail)
        return {"status": "SKIP", "detail": detail}, {"status": "SKIP", "detail": detail}

    if shutil.which(matlab_cmd) is None:
        _warn(
            f"MATLAB not detected ('{matlab_cmd}' is not on PATH), so the MATLAB MEX files "
            "installed into external_programs could not be verified. This does not affect the "
            "Python pipelines. To verify them later, install MATLAB (or pass --matlab-cmd "
            "<path>) and rerun this installer."
        )
        detail = f"MATLAB not detected ('{matlab_cmd}' not in PATH)"
        return {"status": "SKIP", "detail": detail}, {"status": "SKIP", "detail": detail}

    symbols = _probe_matlab_symbols(repo_root, matlab_cmd)
    _log_matlab_symbol_status(symbols)

    missing = [name for name in MATLAB_GPUFIT_SYMBOLS if not bool(symbols.get(name, False))]
    if not missing:
        symbol_status = {"status": "PASS", "detail": "all required MATLAB Gpufit symbols found"}
    elif not _require_matlab_gpufit_by_default(host_key):
        _log("MATLAB Gpufit symbol requirement disabled for this install")
        symbol_status = {
            "status": "SKIP",
            "detail": "missing on macOS (expected for current release assets): " + ", ".join(missing),
        }
    else:
        raise RuntimeError(
    ...
    return symbol_status, _probe_matlab_runtime_gpufit(repo_root, matlab_cmd)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "then write a rocketship launcher for the GUI."
        )
    )
    parser.add_argument(
        "-R",
    ...
    parser.add_argument(
        ...
        "--no-gui",
        action="store_true",
        help="Skip requirements_gui.txt and the rocketship launcher (GUI deps are installed by default).",
    )
    parser.add_argument(
        "-k",
        "--no-sha256",
...
def main() -> int:
    ...
    try:
        ...
        matlab_symbol_status, matlab_runtime_status = _check_matlab_install(
            repo_root=repo_root,
        ...
        launchers: List[Path] = []
        if args.no_gui:
            _log("GUI dependencies skipped; not writing the GUI launchers")
        else:
            launchers = _write_gui_launchers(repo_root, python_in_venv)
        ...
        _print_post_install_next_steps(repo_root, python_in_venv, launchers)
        return 0
