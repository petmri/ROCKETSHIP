...
Gpufit/Cpufit acceleration, and the `rocketship.sh` GUI launcher."""
...
# Generated by install.py. Launches a ROCKETSHIP GUI inside the project virtual
# environment. Re-run "python3 install.py" to regenerate.
set -euo pipefail
...
GUI="run_dce_python_gui.py"
case "${1:-}" in
    dce) shift ;;
    parametric) GUI="run_parametric_python_gui.py"; shift ;;
    -h|--help)
        echo "Usage: $(basename "${BASH_SOURCE[0]}") [dce|parametric] [gui args...]"
        echo "Launches a ROCKETSHIP GUI in the project virtual environment (default: dce)."
        exit 0
        ;;
esac
...
exec python "$REPO_ROOT/$GUI" "$@"
"""


LAUNCHER_BAT = """@echo off
REM Generated by install.py. Launches a ROCKETSHIP GUI inside the project virtual
REM environment. Re-run "python install.py" to regenerate.
setlocal
set "REPO_ROOT=%~dp0"
set "VENV_DIR=__VENV_DIR__"
set "ACTIVATE=%VENV_DIR%\\Scripts\\activate.bat"

set "GUI=run_dce_python_gui.py"
if /I "%~1"=="-h" goto usage
if /I "%~1"=="--help" goto usage
if /I "%~1"=="dce" (shift) else if /I "%~1"=="parametric" (set "GUI=run_parametric_python_gui.py" & shift)
...
call "%ACTIVATE%"
python "%REPO_ROOT%%GUI%"%ARGS%
exit /b %ERRORLEVEL%

:usage
echo Usage: %~nx0 [dce^|parametric] [gui args...]
echo Launches a ROCKETSHIP GUI in the project virtual environment (default: dce).
exit /b 0
"""


def _write_gui_launcher(repo_root: Path, venv_python: Path) -> Path:
    """Write a shell wrapper that activates the venv and launches a ROCKETSHIP GUI."""
    ...
    if os.name == "nt":
        ...
        launcher = repo_root / "rocketship.bat"
        launcher.write_text(LAUNCHER_BAT.replace("__VENV_DIR__", venv_expr), encoding="utf-8")
    else:
        try:
            venv_expr = '"$REPO_ROOT"/' + venv_root.relative_to(repo_root).as_posix()
        except ValueError:
            venv_expr = shlex.quote(str(venv_root))
        launcher = repo_root / "rocketship.sh"
        launcher.write_text(LAUNCHER_SH.replace("__VENV_DIR__", venv_expr), encoding="utf-8")
        launcher.chmod(0o755)
    _log(f"Wrote GUI launcher: {launcher}")
    return launcher
...
def _print_post_install_next_steps(
    ...
    if launcher is not None:
        launcher_cmd = launcher.name if os.name == "nt" else f"./{launcher.name}"
        print(f"  {step}) Launch the GUI: {launcher_cmd}   (from {repo_root})")
        step += 1
...
def main() -> int:
    ...
    try:
        ...
        launcher: Optional[Path] = None
        if args.no_gui:
            _log("GUI dependencies skipped; not writing the GUI launcher")
        else:
            launcher = _write_gui_launcher(repo_root, python_in_venv)
        ...
        _print_post_install_next_steps(repo_root, python_in_venv, launcher)
        return 0
