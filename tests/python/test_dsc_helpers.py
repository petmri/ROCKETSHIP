...
def _compare_nested_lists(test_case: unittest.TestCase, actual, expected, atol: float, rtol: float) -> None:
    if isinstance(expected, list):
        test_case.assertIsInstance(actual, list)
        test_case.assertEqual(len(actual), len(expected))
        for a, e in zip(actual, expected):
            _compare_nested_lists(test_case, a, e, atol, rtol)
        return
...
class TestDscHelpers(unittest.TestCase):
    def test_import_and_previous_match_matlab_baseline(self) -> None:
        ...
        # Reproduce MATLAB synthetic fixture from export_parity_baseline.m
        ...
        _compare_nested_lists(self, import_out[0], expected_import["meanAIF_adjusted"], atol, rtol)
        _compare_nested_lists(self, import_out[1], expected_import["time_vect"], atol, rtol)
        _compare_nested_lists(self, import_out[2], expected_import["concentration_array"], atol, rtol)
        _compare_nested_lists(self, import_out[3], expected_import["meanSignal"], atol, rtol)

        _compare_nested_lists(self, prev_out[0], expected_prev["meanAIF_adjusted"], atol, rtol)
        _compare_nested_lists(self, prev_out[1], expected_prev["time_vect"], atol, rtol)
        _compare_nested_lists(self, prev_out[2], expected_prev["concentration_array"], atol, rtol)

    def test_import_truncates_last_dimension_when_time_longer_than_aif(self) -> None:
        ...
        self.assertEqual(len(adjusted), len(mean_aif) - (bolus_time - 1))
        self.assertEqual(len(out_time), len(adjusted))
        self.assertEqual(last_dim_len(out_conc), len(adjusted))
