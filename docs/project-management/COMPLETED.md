...
- [x] Rebuilt patched `pyCpufit 1.4.1` (Gpufit `dev` `3db5b4d` "Fix false CONVERGED on rejected step" + `607f127` global-convergence) verified in use, and added a **backend-agnostic multi-start** (`python/dce_pipeline.py:_accel_multistart_refine`): the accelerated Stage-D fit re-fits only voxels that pin `vp`/`Fp` to a bound (or fail) from a few perturbed starts and keeps the lowest chi-square, so it can never degrade a good fit and applies identically to cpufit/gpufit. This resolved `tissue_uptake` (2CUM) on the OSIPI sweep (1 failing case → 0); promoted the cpufit/gpufit 2CUM sweep from `xfail` to passing. Residual `2cxm` accelerated precision limitation (float32 `vp`↔`Fp` degeneracy) documented and tracked in `TODO.md` + `docs/project-management/projects/osipi-verification/gpufit_2cxm_2cum_divergence.md`.
- [x] Investigated why the python backend beats the accelerated path on the stiff models: python 2CUM already multi-starts (+ linear-Patlak seed via `_best_fit_over_starts`), python 2CXM wins on float64 + the `E=Ktrans/Fp` reparameterization (single start). Confirmed initialization is not the lever for accelerated 2CXM (warm-start from linear Patlak makes it *worse*). Regenerated `osipi_summary.md` + figures with per-backend numbers.
