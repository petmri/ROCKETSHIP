...
- **Won't do: backend diagnostics surfaced in Python test failure messages.**
      ...
      the state codes.
- [x] **Accelerated 2CXM is worth keeping; the gpufit slowness was root-caused and fixed.**
  Benchmarked python/cpufit/gpufit on CUDA at 24/240/2400 voxels (5 s frames -> 2951 dense
  points, RTX 5090 against one CPU core). Two things corrected the previous reading. cpufit is
  3.6-3.9x faster than python at every size, so "cpufit 2cxm buys nothing" did not hold on this
  host. And gpufit, which was 4.8-8.7x *slower* than a single CPU core, was not losing to the
  O(n^2) CUDA convolution as assumed: `two-compartment_exchange.cuh` accumulated
  `Gpos += 0.5 * spacing * (...)` with a **`double` literal**, promoting the whole inner loop to
  FP64 (1 `DMUL` + 4 `DFMA` + 13 `F2F` per iteration in the SASS) at 1/64 rate on consumer
  Blackwell. `tissue_uptake.cuh` had the same bug. Float literals alone gave 14x; honouring the
  existing `backtrack_accepted` mask in `cuda_calc_curve_values` — profiled at 92.4% of GPU time
  running exactly 9.0 model evaluations per LM iteration — gave a further 1.25-1.56x,
  bit-identical. Net **21.8x** at 2400 voxels, taking gpufit from 4.8x slower than cpufit to 4.6x
  faster; tissue_uptake 9.4x on 600-point curves. Released as Gpufit v1.5.1. Verified with
...
- [x] **Fp *is* identifiable at typical DCE frame rates; the dense grid fixed it.** The previous
  ...
  implementation. Re-measured on the 24 OSIPI 2CXM DRO cases, median |relative error| against
  ground truth: **0.3% at 0.5 s frames, 0.6% at 1 s and 2 s, 0.8% at 5 s, and 567% at 10 s.** The
  cliff is between 5 s and 10 s, not at 5 s. So the problem was never purely information-
  theoretic — it was a discretization error in the forward model, which is what the 0.1 s
  upsample removed (plasma MTT is 1-2 s, far below a 5 s frame, so the convolution was being
  evaluated on a grid that could not represent it). Noise sensitivity at 5 s frames, 24 cases x
  20 realisations: Fp median error 1.2% at SNR ~410, 5.6% at ~40, 11.6% at ~20, 28.7% at ~8 — but
  the tail is heavy throughout (p90 8.5% / 34.9% / 81.8% / 975%), so Fp stays the least
  determined of the four parameters. Caveat on all of the above: the DRO is generated by the
  exact model being fitted, so these are best-case figures with no model mismatch, AIF error or
  motion. The residual actionable piece — a frame-rate/SNR gate on reporting Fp — is carried in
  `TODO.md`.
...
- [x] **One 2CXM implementation, evaluated on the dense grid, on every backend.** Python carried
  ...
  **Both caveats were superseded on 2026-08-12** (see that entry above): cpufit is 3.6-3.9x
  faster than python on a CUDA host, and Fp recovers to 0.8% median error at 5 s frames.
